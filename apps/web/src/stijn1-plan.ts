...
function trainer(b: Blok, optioneel = false): GeplandBlok {
  return { vak: b.vak, vakBlokId: b.id, trainerBlokIds: [b.id], soort: "trainer", ...(optioneel ? { optioneel: true } : {}) };
}
...
export function bouwStijn1Schema(blokken: Blok[], vandaagISO: string, begonnenVandaag: GeplandBlok[] = []): SchemaResultaat {
  ...
  // [datum, ISO] → blokken (niet-optioneel eerst, optioneel achteraan).
  const dagen: Record<string, GeplandBlok[]> = {
    // wo 24/6 — vandaag: begonnen werk blijft + Franse werkwoorden voor de rest.
    "2026-06-24": [
      ...begonnenVandaag,
      ...pick("frans/werkwoorden/h6").map((b) => trainer(b)),
    ],
    // do 25/6 — Frans (vocab) · Geschiedenis deel 1 · Wiskunde.
    "2026-06-25": [
      ...fransVocab().map((b) => trainer(b)),
      ...geschDeel1.map((b) => trainer(b)),
      wiskundeBoek("2026-06-25"),
    ],
    // vr 26/6 — Geschiedenis deel 2 · Frans (vertaalzinnen) · Wiskunde.
    "2026-06-26": [
      ...geschDeel2.map((b) => trainer(b)),
      ...pick("frans/vertalen/h6").map((b) => trainer(b)),
      wiskundeBoek("2026-06-26"),
    ],
    // za 27/6 — Frans (cloze) · Biologie · Wiskunde · Geschiedenis (optioneel, achteraan).
    "2026-06-27": [
      ...pick("frans/invullen/h6").map((b) => trainer(b)),
      ...vak("biologie").map((b) => trainer(b)),
      wiskundeBoek("2026-06-27"),
      ...geschAlles.map((b) => trainer(b, true)),
    ],
    // zo 28/6 — Engels · Wiskunde · Handvaardigheid (zelf afvinken) · Aardrijkskunde (optioneel).
    "2026-06-28": [
      ...vak("engels").map((b) => trainer(b)),
      wiskundeBoek("2026-06-28"),
      afvink("handvaardigheid", "handvaardigheid:2026-06-28", "Handvaardigheid & tekenen (zelfstudie)"),
      ...vak("aardrijkskunde").map((b) => trainer(b, true)),
    ],
    // ma 29/6 (engels-toets) — Biologie · Wiskunde.
    "2026-06-29": [
      ...vak("biologie").map((b) => trainer(b)),
      wiskundeBoek("2026-06-29"),
    ],
    // di 30/6 (bio-toets) — Nederlands · Wiskunde · Frans (herhaling).
    "2026-06-30": [
      ...vak("nederlands").map((b) => trainer(b)),
      wiskundeBoek("2026-06-30"),
      ...fransAlles().map((b) => trainer(b)),
    ],
    // wo 1/7 (ndl-toets) — Aardrijkskunde · Wiskunde · Geschiedenis (chronologie-herhaling).
    "2026-07-01": [
      ...vak("aardrijkskunde").map((b) => trainer(b)),
      wiskundeBoek("2026-07-01"),
      ...geschChronologie.map((b) => trainer(b)),
    ],
    // do 2/7 (wisk.+ak-toets) — Frans (alles) · Geschiedenis (herhaling: alle onderdelen).
    "2026-07-02": [
      ...fransAlles().map((b) => trainer(b)),
      ...geschAlles.map((b) => trainer(b)),
    ],
  };

  const toewijzingen: DagToewijzing[] = Object.entries(dagen)
    .filter(([, bl]) => bl.length > 0)
    .map(([datum, bl]) => ({ datum, type: "leer", toetsVakken: [], blokken: bl, cap: bl.length }));
